...
Unicast sends a typed message from a `PulseViewModel` up to its parent `PulseContainer`.
...
Implement `PulseUnicast` with a sealed interface or sealed class. The Unicast type is shared by the Container and every ViewModel registered in it. This generic pairing means a ViewModel cannot emit a Unicast type that the Container does not understand.
...
Override `onReceived()` in the Container. In the example below, the Container takes the Unicast it received from a ViewModel and sends it back out to all ViewModels as a Broadcast.
...
Two counter ViewModels keep separate local repositories, yet share updates through their parent Container:
